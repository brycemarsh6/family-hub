// The test recipes shared by seed-recipes.ts and clean-recipes.ts.
// Lives in its own file so the cleanup script can know exactly which
// rows are test data without importing (and thereby running) the seed
// script. Both scripts match on these titles rather than clearing the
// whole Recipe table -- it holds the family's real recipes now.
